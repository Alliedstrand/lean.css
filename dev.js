...
app.get("/", async (req, res) => {
  ...
  const injected = html
    .toString()
    .replace("</body>", `<script src='/socket.io/socket.io.js'></script><script>${script}</script></body>`);
  res.send(injected);
});
...
chokidar.watch("src/partials/*.scss", { ignoreInitial: true }).on("all", async () => {
  console.log("> Stylesheet partial changed");
  const src = await fs.readdir("src");
  const files = src.filter(file => file !== "partials").map(file => `src/${file}`);
